...
private:
  ...
  TableMetadata(table_id_t table_id, std::string table_name, page_id_t root_page_id, TableSchema *schema,vector<uint32_t>primary_key_);
...
public:
  ...
  inline vector<uint32_t> Getpri() const {return table_meta_->primary_key;}
private:
