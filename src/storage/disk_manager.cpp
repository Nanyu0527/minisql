...
page_id_t DiskManager::AllocatePage() {
  /*uint32_t extent_id, extent_page_num, i, extent_max_page;
  DiskFileMetaPage *metapage = new DiskFileMetaPage;
  extent_id = metapage->GetExtentNums();
  extent_max_page = BitmapPage<PAGE_SIZE>::GetMaxSupportedSize();
  for(i=1; i<extent_id; i++)
  {
    extent_page_num = metapage->GetExtentUsedPage(extent_id);
    if(extent_page_num < extent_max_page)
      break;
  }
  metapage->num_allocated_pages_++;
  return (i*extent_max_page + metapage->GetAllocatedPages()-1);*/
  DiskFileMetaPage *meta_page = reinterpret_cast<DiskFileMetaPage *>(meta_data_);
  uint32_t extent_num, /*extent_max_page,*/ ofs, i;
  char bitmap_data_[PAGE_SIZE];

  //extent_max_page = BitmapPage<PAGE_SIZE>::GetMaxSupportedSize();
  if(!meta_page->GetExtentNums())
    meta_page->num_extents_++;
  extent_num = meta_page->GetExtentNums();
  for(i=0; i<extent_num; i++)
  {
    if(meta_page->GetExtentUsedPage(i) < BITMAP_SIZE)
      break;
  }
  if(i == extent_num && meta_page->GetExtentUsedPage(i-1) == BITMAP_SIZE)
  {
    meta_page->num_extents_++;
  }
  int BITMAP_PAGE_ID = i*(BITMAP_SIZE + 1) + 1;
  ReadPhysicalPage(BITMAP_PAGE_ID, bitmap_data_);
  ...
  if(bitmap->AllocatePage(ofs))
  {
    ...
    WritePhysicalPage(BITMAP_PAGE_ID, bitmap_data_);
    return i*BITMAP_SIZE+ofs;
  }
  return 0;
}

void DiskManager::DeAllocatePage(page_id_t logical_page_id) {
  //page_id_t logical_id;
  /*uint32_t offset;
  page_id_t physical_id = MapPageId(logical_page_id);
  offset = (physical_id) * PAGE_SIZE;
  if(bitmap.DeAllocatePage(offset)) return;
  ASSERT(false, "Not implemented yet.");*/
  DiskFileMetaPage *meta_page = reinterpret_cast<DiskFileMetaPage *>(meta_data_);
  uint32_t extent_id, ofs, extent_max_page;
  char bitmap_data_[PAGE_SIZE];
  extent_max_page = BitmapPage<PAGE_SIZE>::GetMaxSupportedSize();
  extent_id = logical_page_id / extent_max_page;
  ofs = logical_page_id % extent_max_page;

  int BITMAP_PAGE_ID = extent_id*(extent_max_page + 1) + 1;
  ReadPhysicalPage(BITMAP_PAGE_ID, bitmap_data_);
  BitmapPage<PAGE_SIZE> *bitmap = reinterpret_cast<BitmapPage<PAGE_SIZE> *>(bitmap_data_);
  bitmap->DeAllocatePage(ofs);
  WritePhysicalPage(BITMAP_PAGE_ID, bitmap_data_);
  meta_page->num_allocated_pages_--;
...
bool DiskManager::IsPageFree(page_id_t logical_page_id) {
  /*page_id_t physical_id = MapPageId(logical_page_id);
  uint32_t offset = physical_id * PAGE_SIZE;
...
page_id_t DiskManager::MapPageId(page_id_t logical_page_id) {
  ...
  return extent_id*(extent_max_page + 1) + ofs + 2;
}
